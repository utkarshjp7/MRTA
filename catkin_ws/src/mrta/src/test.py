...
def print_graph(graph):
    ...
    print "--------First Layer----------"
    tf = graph.get_first_layer()

    for v in tf:
        print str(v)

    print "--------Second Layer----------"
    tl = graph.get_second_layer()

    for v in tl:
        print str(v)

    print "--------Hidden Layer----------"
    th = graph.get_hidden_layer()
...
graph_1 = PrecedenceGraph([x, y, z, a, b, c, d])
...
graph_2 = dg.generate_precedence_graph(tasks, 20)
