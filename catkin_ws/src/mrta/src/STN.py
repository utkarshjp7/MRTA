...
class STN(object):
    ...
    def __init__(self, robot_pos, robot_speed):
        ...
        self._digraph.add_node("zero", type="zero", task=zero_task, index=-1)
    ...
    def insert_task(self, task, index=None):
        ...
        self._digraph.add_node(start_node_id, type='start', task=task, index=self.task_count)
        self._digraph.add_node(end_node_id, type='end', index=self.task_count)
        ...
        if index == 0:
            zero_node = self._get_node(-1)
            zero_node_task = self._get_task(-1)
            travel_time = self._compute_travel_time(zero_node_task.location, task.location) 
            additional_travel_time += travel_time

        elif index == self.task_count:
            last_node = self._get_node(self.task_count-1)
            last_node_task = self._get_task(self.task_count-1)
            travel_time = self._compute_travel_time(last_node_task.location, task.location) 
            self._add_temporal_constraint(last_node[0], start_node_id, travel_time, travel_time)
            additional_travel_time += travel_time

        else:
            previous_node = self._get_node(index-1, type='end')
            ...
            next_node = self._get_node(index, type='start')
            next_node_task = self._get_task(index)
    ...
    def is_consistent(self):
        ...
        """        
        #return not nx.negative_edge_cycle(self._digraph)
    ...
    def solve_stn(self, pc):
        ...
        for i in range(self.task_count):
            ...
            start_time = max(pc, cur_task.est, pre_task.finish_time + travel_time)
    ...
    def _get_node(self, index=None, task_id=None, _type=None):
        ...
        if index is not None:
            ...
            _type = None
            result = filter(lambda (n,d): d['task'].id == task_id, self._digraph.nodes(data=True))        
